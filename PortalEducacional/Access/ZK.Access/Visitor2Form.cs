...
namespace ZK.Access
{
	public class Visitor2Form : UserControl
	{
		...
		private string _lastDataFilter = null;
		...
		private GridColumn column_visitstatus;
		...
		private void InitDataTableSet()
		{
			...
			this.m_datatable.Columns.Add("visitstatus");
			...
			this.column_visitstatus.FieldName = "visitstatus";
		...
		private void Add_button_Click(object sender, EventArgs e)
		{
			if (!this.isDouble)
			{
				...
				try
				{
					...
					Visitor2ManagementForm visitor2ManagementForm = new Visitor2ManagementForm(0, 0, "0");
					visitor2ManagementForm.refreshDataEvent += this.addDept_RefreshDataEvent;
					visitor2ManagementForm.ShowDialog();
					visitor2ManagementForm.refreshDataEvent -= this.addDept_RefreshDataEvent;
				}
		...
		private void addDept_RefreshDataEvent(object sender, EventArgs e)
		{
			...
			if (this.m_datatable.Rows.Count > 0)
			{
				...
				int num = 0;
				int num2 = 0;
				if (sender is Visitor2ManagementForm)
				{
					num = (sender as Visitor2ManagementForm).Add_Event_Userid;
				}
		...
		{
			this.DataBind(null, false, 0);
		}

		private void DataBind(string dataFilter, int value = 0)
		{
			this.DataBind(dataFilter, false, value);
		}

		private void DataBind(string dataFilter, bool onlyactive, int value = 0)
		{
			if (dataFilter != null)
			{
				this._lastDataFilter = dataFilter;
			}
			try
			{
				UserInfoBll.ClearDs();
				...
				DataSet dataSet = (!(this._lastDataFilter == "")) ? this.bll.GetList(this._lastDataFilter, "S", onlyactive, false) : this.bll.GetAllList("S", false);
				...
				if (dataSet != null && dataSet.Tables.Count > 0)
				{
					...
					if (this.m_datatable.Rows.Count > 0 && value > 0)
					{
						...
						if (array.Length != 0 && array2.Length != 0)
						{
							...
							array[0]["visitstatus"] = array2[0]["visitstatus"];
						...
						else
						{
							...
							dataRow[5] = "?";
							dataRow[6] = array2[0]["visitstatus"];
							if (array2[0]["Gender"].ToString().Trim() == "F")
							{
								dataRow[7] = value3;
							}
					...
					else
					{
						...
						for (int i = 0; i < dataTable.Rows.Count; i++)
						{
							...
							dataRow2[5] = "??";
							dataRow2[6] = dataTable.Rows[i]["visitstatus"].ToString();
					...
					if (this.m_datatable.Rows.Count > 0)
					{
						...
						if (array != null && array.Length != 0 && array[0] >= 0 && array[0] < this.m_datatable.Rows.Count)
						{
							if (array.Length == 1)
							{
								...
								string text = this.m_datatable.Rows[array[0]][5].ToString();
								Program.IsRegistZKECardTong();
								Visitor2ManagementForm visitor2ManagementForm = new Visitor2ManagementForm(int.Parse(s), 0, this.dept);
								visitor2ManagementForm.refreshDataEvent += this.addDept_RefreshDataEvent;
								visitor2ManagementForm.ShowDialog();
								visitor2ManagementForm.refreshDataEvent -= this.addDept_RefreshDataEvent;
							}
		...
		{
			DialogResult dialogResult = SysDialogs.ShowQueseMessage("Deseja encerrar as visitas selecionadas?", MessageBoxButtons.YesNo, MessageBoxDefaultButton.Button2);
			if (dialogResult != DialogResult.No)
			{
				...
				try
				{
					...
					if (checkedRows == null)
					{
						SysDialogs.ShowInfoMessage("Selecione pelo menos um registro");
					}
					else
					{
						if (checkedRows != null && checkedRows.Length != 0 && checkedRows[0] >= 0 && checkedRows[0] < this.m_datatable.Rows.Count)
						{
							...
							{
								string text = this.m_datatable.Rows[checkedRows[i]]["USERID"].ToString();
								string text2 = this.m_datatable.Rows[checkedRows[i]]["visitstatus"].ToString();
								if (text2.Equals("VISITA ATIVA"))
								{
									List<UserInfo> list = new List<UserInfo>();
									VisitInfo model = dALObject.GetModel(int.Parse(text));
									...
									if (model != null)
									{
										...
										list.Add(userInfo);
										list2 = machinesBll.GetModelList("id in(select  device_id from acc_door where id in(select  accdoor_id from acc_levelset_door_group where acclevelset_id in(select acclevelset_id from acc_levelset_emp where employee_id=" + text + ")))");
										DevCmds.RunImmediatelyEnabled = true;
										for (int j = 0; j < list2.Count; j++)
										{
											string text3;
											if (list2[j].DevSDKType != SDKType.StandaloneSDK)
											{
												CommandServer.DeleteFace(list2[j], list, true, out text3);
												CommandServer.DeleteFingerVein(list2[j], list, true, out text3);
												CommandServer.DeleteFingerPrint(list2[j], list, true, out text3);
												CommandServer.DeleteUserAuthorize(list2[j], list, true, out text3);
											}
											CommandServer.DeleteUser(list2[j], list, true, out text3);
										}
										DevCmds.RunImmediatelyEnabled = false;
									}
									dALObject.EndVisit(int.Parse(text));
									personnelIssuecardBll.DeleteByUserId(int.Parse(text));
								}
							}
						}
						this.DataBind("", 0);
			...
			if (base.Visible && !base.IsDisposed)
			{
				try
				{
					...
					else
					{
						...
						{
							this.bll.Add(this.m_addUsers, "S");
							this.m_addUsers.Clear();
						}
		...
		private void import_SetImportColumnsEvent(DataConfig config)
		{
			if (config != null)
			{
				try
				{
					...
					config.ImportColumns.Add(ShowMsgInfos.GetInfo("UserNo", "Pin"));
					config.ImportColumns.Add(ShowMsgInfos.GetInfo("CardNo", "Cartão"));
					config.ImportColumns.Add(ShowMsgInfos.GetInfo("lastName", "Sobrenome"));
					if (initLang.Lang.ToLower() != "chs")
					{
						config.ImportColumns.Add(ShowMsgInfos.GetInfo("FirstName", "Nome"));
					}
					config.ImportColumns.Add(ShowMsgInfos.GetInfo("DeptNo", "Departamento"));
					config.ImportColumns.Add(ShowMsgInfos.GetInfo("sex", "Sexo"));
					config.ColumnsToModelDic.Add(ShowMsgInfos.GetInfo("UserNo", "Pin"), "Badgenumber");
					config.ColumnsToModelDic.Add(ShowMsgInfos.GetInfo("CardNo", "Cartão"), "CardNo");
					config.ColumnsToModelDic.Add(ShowMsgInfos.GetInfo("lastName", "Sobrenome"), "Name");
					if (initLang.Lang.ToLower() != "chs")
					{
						config.ColumnsToModelDic.Add(ShowMsgInfos.GetInfo("FirstName", "Nome"), "lastname");
					}
					config.ColumnsToModelDic.Add(ShowMsgInfos.GetInfo("DeptNo", "Departamento"), "DeptName");
					config.ColumnsToModelDic.Add(ShowMsgInfos.GetInfo("sex", "Sexo"), "gender");
				}
				catch (Exception ex)
				{
					if (this.import != null)
		...
		private void btn_search_Click_1(object sender, EventArgs e)
		{
			try
			{
				PersonnelSearchForm personnelSearchForm = new PersonnelSearchForm(true);
				...
				if (personnelSearchForm.DialogResult == DialogResult.OK)
				{
					...
					this.DataBind(" DEFAULTDEPTID <> -1 " + this.strCondtion, personnelSearchForm.getOnlyActive(), 0);
				}
			}
			catch (Exception ex)
			{
		...
		{
			DialogResult dialogResult = SysDialogs.ShowQueseMessage("Deseja reenviar as visitas selecionadas?", MessageBoxButtons.YesNo, MessageBoxDefaultButton.Button2);
			if (dialogResult != DialogResult.No)
			{
				IVisitInfo dALObject = VisitInfoBll.getDALObject();
				MachinesBll machinesBll = new MachinesBll(MainForm._ia);
						...
						if (checkedRows != null && checkedRows.Length != 0 && checkedRows[0] >= 0 && checkedRows[0] < this.m_datatable.Rows.Count)
						{
							for (int i = 0; i < checkedRows.Length && checkedRows[i] >= 0 && checkedRows[i] < this.m_datatable.Rows.Count; i++)
							{
								...
								if (text2.Equals("VISITA ATIVA"))
								{
									...
									{
										UserInfo userInfo = new UserInfo();
										userInfo.UserId = int.Parse(text);
										userInfo.BadgeNumber = model.pin;
										userInfo.CardNo = model.card;
										list.Add(userInfo);
										list2 = machinesBll.GetModelList("id in(select  device_id from acc_door where id in(select  accdoor_id from acc_levelset_door_group where acclevelset_id in(select acclevelset_id from acc_levelset_emp where employee_id=" + text + ")))");
										...
										for (int j = 0; j < list2.Count; j++)
										{
											this.SendVisitInfo(userInfo, list2[j]);
										}
										DevCmds.RunImmediatelyEnabled = false;
									}
								}
		...
		private void SendVisitInfo(UserInfo newUser, Machines machine)
		{
			...
			{
				CommandServer.DeleteFace(machine, list, true, out text);
				CommandServer.DeleteFingerVein(machine, list, true, out text);
				CommandServer.DeleteFingerPrint(machine, list, true, out text);
				CommandServer.DeleteUserAuthorize(machine, list, true, out text);
			}
			CommandServer.DeleteUser(machine, list, true, out text);
			CommandServer.AddUser(machine, list, dictionary3);
			CommandServer.AddUserPhoto(machine, list);
			CommandServer.AddUserVerifyType(machine, list2);
			CommandServer.AddFingerPrint(machine, list);
			CommandServer.AddFingerVein(machine, list);
			CommandServer.AddFace(machine, list);
			CommandServer.AddElevator(machine, list, dictionary3);
			modelList4 = accLevelsetBll.GetModelList("id in (select acclevelset_id from acc_levelset_door_group where accdoor_device_id=" + machine.ID + ") and id in(select acclevelset_id from acc_levelset_emp where employee_id=" + newUser.UserId + ")", true);
			if (modelList4 != null)
			{
				for (int m = 0; m < modelList4.Count; m++)
				{
					modelList = accDoorBll.GetModelList("device_id = " + machine.ID + " and id in (select accdoor_id from acc_levelset_door_group where acclevelset_id = " + modelList4[m].id + ")");
					if (modelList == null || modelList.Count <= 0)
					{
						break;
					}
					CommandServer.AddUserAuthorize(machine, dictionary4[modelList4[m].id], list, modelList);
				}
			}
		}
		...
		private void InitializeComponent()
		{
			...
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(Visitor2Form));
			...
			this.column_visitstatus = new GridColumn();
			...
			this.contextMenuStrip1.Size = new Size(145, 92);
			this.Menu_add.Image = (Image)componentResourceManager.GetObject("Menu_add.Image");
			this.Menu_add.Name = "Menu_add";
			this.Menu_add.Size = new Size(144, 22);
			this.Menu_add.Text = "Adiciona";
			...
			this.Menu_edit.Size = new Size(144, 22);
			this.Menu_edit.Text = "Edita";
			...
			this.Menu_delete.Size = new Size(144, 22);
			this.Menu_delete.Text = "Apaga";
			...
			this.Menu_adjustDept.Size = new Size(144, 22);
			this.Menu_adjustDept.Text = "Encerra Visita";
			this.Menu_adjustDept.Click += this.btn_adjustDept_Click;
			this.MenuPanelEx.AutoSize = false;
			this.MenuPanelEx.Items.AddRange(new ToolStripItem[10]
			{
				this.btn_add,
				this.btn_edit,
				this.btn_delete,
			...
			this.btn_add.Size = new Size(90, 38);
			...
			this.btn_edit.Size = new Size(69, 38);
			this.btn_edit.Text = "Edita";
			...
			this.btn_search.Size = new Size(84, 38);
			this.btn_search.Text = "Procura";
			this.btn_search.Click += this.btn_search_Click_1;
			this.btn_adjustDept.Image = Resources.devDisable;
			...
			this.btn_adjustDept.Size = new Size(97, 38);
			this.btn_adjustDept.Text = "Encerra Visita";
			...
			this.btn_batchAddPersonnel.Visible = false;
			...
			this.btn_import.Size = new Size(67, 36);
			...
			this.btn_export.Size = new Size(67, 36);
			...
			this.pnl_personnel.Text = "Visitantes";
			...
			this.column_badgenumber.Caption = "No. Identificação";
			...
			this.column_cardno.Caption = "Cartão";
			...
			this.column_code.Caption = "Código";
			this.column_code.Name = "column_code";
			this.column_code.Width = 73;
			this.column_visitstatus.Caption = "Status";
			this.column_visitstatus.Name = "column_visitstatus";
			this.column_visitstatus.Visible = true;
			this.column_visitstatus.VisibleIndex = 5;
			this.column_visitstatus.Width = 73;
			this.column_gender.Caption = "Sexo";
			this.column_gender.Name = "column_gender";
			this.column_gender.Visible = true;
			this.column_gender.VisibleIndex = 6;
			this.column_gender.Width = 73;
			this.column_acount.Caption = "ID V10.0";
			this.column_acount.Name = "column_acount";
			this.column_acount.Visible = true;
			this.column_acount.VisibleIndex = 7;
			this.column_acount.Width = 80;
			this.colFp9Count.Caption = "ID V9.0";
			this.colFp9Count.Name = "colFp9Count";
			this.colFp9Count.Visible = true;
			this.colFp9Count.VisibleIndex = 8;
			this.column_acountFv.Caption = "I.V.";
			this.column_acountFv.Name = "column_acountFv";
			this.column_acountFv.Visible = true;
			this.column_acountFv.VisibleIndex = 9;
			this.column_acountFv.Width = 80;
			this.colPullFaceCount.Caption = "Face";
			this.colPullFaceCount.Name = "colPullFaceCount";
			this.colPullFaceCount.Visible = true;
			this.colPullFaceCount.VisibleIndex = 10;
			...
			this.toolStripButton1.Size = new Size(115, 38);
			this.toolStripButton1.Text = "Reenvia Visita";
